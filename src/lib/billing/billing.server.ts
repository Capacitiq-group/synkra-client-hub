...
export async function resolveOrCreateUser(
  ...
  const record = await pb.collection("users").create({
    ...
    tier: "free",
    // "beta" is the free-tier value ("beta" | "paid" per src/lib/auth.ts);
    // applyEntitlement flips this to "paid" only when a purchasable tier is
    // actually applied. Hardcoding "paid" here mislabeled every brand-new
    // free-tier signup as PRO in the dashboard badge.
    user_type: "beta",
...
export async function applyEntitlement(
  ...
  await pb.collection("users").update(input.userId, {
    tier: input.tier,
    user_type: isPurchasableTier(input.tier) ? "paid" : "beta",
