...
export async function startExecution(input: StartExecutionInput): Promise<StartExecutionResult> {
  ...
  // Reserve one execution against the monthly counter FIRST, atomically
  // (PocketBase's "+"-suffixed update, applied at the DB layer). The earlier
  // `decision.allowed` snapshot can go stale under concurrency — two requests
  // landing near the limit could both read the same pre-reservation count and
  // both believe they're allowed. Reserving first and checking the post-increment
  // value closes that window: whichever request(s) actually push the counter
  // over the limit get rolled back and rerouted to purchased-credit
  // consumption (or blocked), instead of both proceeding.
  const limit = decision.limit;
  const reserved = (await pb.collection("users").update(input.userId, {
    "executions_used_this_month+": 1,
    billing_period_start: usage.billingPeriodStart,
  })) as unknown as Record<string, unknown>;
  const usedAfterReserve = safeUsage(reserved["executions_used_this_month"]);
  const withinMonthlyLimit = usedAfterReserve <= limit;

  if (!withinMonthlyLimit) {
    // This request's reservation overshot the monthly allowance: give it back
    // and fall through to the purchased-credit / blocked path below, exactly
    // as if `decision.allowed` had correctly been false from the start.
    await pb.collection("users").update(input.userId, { "executions_used_this_month+": -1 });
  ...
  // Allowed: the reservation above already counted this execution. Open the
  // run log against it.
