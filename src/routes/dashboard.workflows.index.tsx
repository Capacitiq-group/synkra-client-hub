...
function WorkflowsPage() {
  ...
  const filtersActive = query.trim() !== "" || category !== "all" || platform !== "all";

  const visibleTemplates = useMemo(
    () => templates.filter((template) => matchesFilters(template, filters)),
    [templates, query, category, platform],
  );
  ...
  const visibleWorkflows = useMemo(() => {
    ...
    return sorted;
  }, [workflowsWithCategory, statusFilter, query, category, platform, sort]);
